...
const Danbooru = Booru.forSite('db');
...
class DanbooruTag {
	...
	async searchTag () {
		...
		try {
			...
			this.imagesRequested = await Danbooru.search([tagToSearch], {
				limit: 100
			});
...
module.exports = {
    ...
    .setName('danbooru')
    .setDescription('Busca y retorna Post de imágenes de Danbooru a partir de un o dos Tag.')
	...
	.addStringOption(option =>
		...
		.setRequired(false)),
    aliases: ['danbooru'],
    description: 'Busca y retorna Post de imágenes de Danbooru a partir de un o dos Tag. \n\n_Busca imágenes de las etiquetas "40hara" y "kinako", ¡te van a gustar!_',
    category: '🔞 NSFW',
    args : '<Tag> [Tag2]',
    async execute(interaction) {		
		await interaction.deferReply();

		let instance = new DanbooruTag(interaction)
