...
class KonachanTag {
	constructor (interaction) {
		...
		this.fixedMessage;
		this.currentPage = 0;
		this.baseKonachanURL = 'https://konachan.com/post/show/';
		this.embedObject = {
            title: null,
            description: "",
            author: {
                name: "Resultados de Konachan",
	...
	async searchTag () {
		...
		try {
			...
			Konachan = Booru.forSite('kc');
			
			this.imagesRequested = await Konachan.search([tagToSearch], {
				limit: 100
			});
	...
	async sendImage (imageOBJ, pageNumber, editEmbed=false) {
		try {
			...
			this.embedObject.description = `[Clic para ir al post original](${this.baseGelbooruURL}${imageOBJ.id})`;
	...
	async waitForUserInput (pageNumber) {
		try {
			...
			const timer = setTimeout( async () => {
				try {
					this.fixedMessage.edit({components:[this.disabledRow]});
				} catch (error) {
...
module.exports = {
    ...
    .setName('konachan')
    .setDescription('Busca y retorna Post de imágenes de Konachan a partir de un o dos Tag.')
	.addStringOption(option =>
	    option
        .setName('tag_one')
		.setDescription('El Tag a buscar en Konachan')
		.setRequired(true))
	.addStringOption(option =>
		option
		.setName('tag_two')
		...
		.setRequired(false)),
    aliases: ['konachan'],
    description: 'Busca y retorna Post de imágenes de Konachan a partir de un o dos Tag. \n\n_Busca imágenes de las etiquetas "40hara" y "kinako", ¡te van a gustar!_',
    category: '🔞 NSFW',
    args : '<Tag> [Tag]',
    async execute(interaction) {
        ...
        let instance = new KonachanTag(interaction)
        instance.searchTag();
    },
};