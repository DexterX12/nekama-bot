...
const Gelbooru = Booru.forSite('gb');
...
class GelbooruTag {
	constructor (interaction) {
		...
		this.fixedMessage;
		this.currentPage = 0;
		this.baseGelbooruURL = 'https://gelbooru.com/index.php?page=post&s=view&id=';
		this.embedObject = {
            title: null,
            description: "",
            author: {
                name: "Resultados de Gelbooru",
	...
	async searchTag () {
		...
		try {
			...
			this.imagesRequested = await Gelbooru.search([tagToSearch], {
				limit: 100
			});
...
module.exports = {
    ...
    .setName('gelbooru')
    .setDescription('Busca y retorna Post de imágenes de Gelbooru a partir de un o dos Tag.')
	.addStringOption(option =>
	    option
        .setName('tag_one')
		.setDescription('El Tag a buscar en Gelbooru')
		.setRequired(true))
	.addStringOption(option =>
		option
		.setName('tag_two')
		...
		.setRequired(false)),
    aliases: ['gelbooru'],
    description: 'Busca y retorna Post de imágenes de Gelbooru a partir de un o dos Tag. \n\n_Busca imágenes de las etiquetas "40hara" y "kinako", ¡te van a gustar!_',
    category: '🔞 NSFW',
    args : '<Tag> [Tag2]',
    async execute(interaction) {
		await interaction.deferReply();

		let instance = new GelbooruTag(interaction)
		await instance.searchTag();
    },
};