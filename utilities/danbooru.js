...
class Danbooru {
	...
	async search (random=false, nsfw=false) {
		...
		if (nsfw) {
			...
			this.posts = await booru.posts({limit: 100, tags:this.safeSearchConfig});
			this.embedOptions.author.name += " con SafeSearch"
		}

		if (random) {
			this.post = this.posts[Math.floor(Math.random() * this.posts.length)];
		} else {
			if (parseInt(index) === "NaN" || !index) {
				this.post = this.posts[0];
			} else {
